...
//    if(nrf_gpio_pin_read(FRAME_VLD)){
//        line_count = 0;
//        m_length_rx_done = 0;
//        lvld_timer_run(&TIMER_LVLD);
...
//    nrf_drv_gpiote_in_config_t in_config_frmvld = NRFX_GPIOTE_CONFIG_IN_SENSE_TOGGLE(true);// changed it to toggle to accomodate the motor! when comes up takes 
    //picture and when comes down starts the timer for motor activation
